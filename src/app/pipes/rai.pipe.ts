...
export class RaiPipe implements PipeTransform {
  ...
  transform(value: any, args?: any): any {
    ...
    switch (denomination.toLowerCase()) {
      default:
      case 'xrb': return `${(value / this.mrai).toFixed(6)}${!hideText ? ' TROLLAR': ''}`;
      case 'mnano':
        ...
        if (hasRawValue) {
          ...
          return `${this.toFixed(newVal, this.precision)}${!hideText ? ' TROLLAR': ''}`;
        } else {
          return `${(value / this.mrai).toFixed(6)}${!hideText ? ' TROLLAR': ''}`;
        }
      case 'ktroll': return `${(value / this.krai).toFixed(3)}${!hideText ? ' ktroll': ''}`;
      case 'troll': return `${(value / this.rai).toFixed(0)}${!hideText ? ' troll': ''}`;
