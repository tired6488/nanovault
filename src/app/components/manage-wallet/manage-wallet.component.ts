...
export class ManageWalletComponent implements OnInit {
  ...
  async exportAddressBook() {
    ...
    const exportUrl = `https://vault.trollar.life/import-address-book#${base64Data}`;
  ...
  exportAddressBookToFile() {
    if (this.walletService.walletIsLocked()) return this.notifications.sendWarning(`Wallet must be unlocked`);
    const fileName = `TrollarVault-AddressBook.json`;
  ...
  exportToFile() {
    ...
    const fileName = `TrollarVault-Wallet.json`;
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        if (!importData.length || !importData[0].account) {
          return this.notifications.sendError(`Bad import data, make sure you selected a TrollarVault Address Book export`)
        }
